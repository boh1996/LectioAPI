...
activity_updated_regex = r"Indlæst: (?P<created_date>[0-9]*)/(?P<created_month>[0-9]*)-(?P<created_year>[0-9]*) (?P<created_hour>[0-9]*):(?P<created_minute>[0-9]*) af (?P<created_teacher>.*)\sSidst rettet: (?P<updated_date>[0-9]*)/(?P<updated_month>[0-9]*)-(?P<updated_year>[0-9]*) (?P<updated_hour>[0-9]*):(?P<updated_minute>[0-9]*) af (?P<updated_teacher>.*)"
activity_homework_regexs = [
    {"name" : "book", "expression" : r"(?P<class>.*) (?P<subject>.*): (?P<writers>.*): ?(?P<name>.*), ?(?P<pages>[0-9]*-[0-9]*) \(kernestof\)\s(?P<note>.*)"},
    {"name" : "note", "expression" : "(?P<npte>.*)"}
]
