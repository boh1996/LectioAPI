        ...
        if index > 1:
            ...
            if not row == None and len(row) > 0:
                for element in row:
                    ...
                    dayOfWeek = index-1
                    date = str(mktime(time.strptime("%s %s %s" % (dayOfWeek,timeWeek, year),"%w %W %Y")))[:-2]
    ...
    for dayElement in dayElements:
        ...
        for timetableElement in timetableElements:
            ...
            timeElements.append({
                ...
                "teachers" : teachers,
                "teams" : teams,
                "startTime" : str(mktime(time.strptime("%s %s %s %s %s" % (timeGroups.group("start_hour"),timeGroups.group("start_minute"), dayOfWeek , timeWeek, year),"%H %M %w %W %Y")))[:-2],
                "endTime" : str(mktime(time.strptime("%s %s %s %s %s" % (timeGroups.group("end_hour"),timeGroups.group("end_minute"), dayOfWeek , timeWeek, year),"%H %M %w %W %Y")))[:-2],
                "type" : type,
